from langchain.chat_models import ChatOpenAI, ChatGoogleGenerativeAI, ChatVertexAI, ChatAnthropic
...
from .tools.agents import web_search_agent_tool, calculator_agent_tool, execute_code_agent_tool
...
class AgentManager:
    ...
    def get_chat_model(self):
        ...
        elif model_preference == 'google':
            return ChatGoogleGenerativeAI(
                model=self.user.preferences.get('google_model', 'gemini-pro'),
                google_api_key=self.user.secrets.get('google_api_key')
            )
        ...
        elif model_preference == 'vertex':
            return ChatVertexAI(
                model_name=self.user.preferences.get('vertex_model', 'chat-bison'),
                project=self.user.preferences.get('gcp_project'),
                location=self.user.preferences.get('gcp_location'),
                credentials=self.user.secrets.get('gcp_credentials')
            )
