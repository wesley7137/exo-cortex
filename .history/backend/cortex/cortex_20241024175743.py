...
class Cortex:
    ...
    def add_to_memory(self, user_prompt: str, model_response: str, context: str = None, data_items: List[Dict] = None, retrieved_memory: List[Dict] = None):
        try:
            ...
            metadata = {
                ...
                "context": {
                    ...
                    "retrieved_memory": retrieved_memory if retrieved_memory else [],
            ...
            self.memory_store.add(
                ...
                embedding_data=[conversation_text],
                metadata=[metadata]
            )
            logger.info("Added conversation to memory successfully.")
        except Exception as e:
...
def cortext_ingest_process_respond(user_id: str, data_items: List[Tuple[str, str, Dict[str, Any]]], user_query: str):
    cortex = Cortex(user_id=user_id)    
    cortex.ki.ingest_batch(data_items)
    
    # Get response
    response = cortex.respond_to_query(user_query)
    
    # Add response to memory
    cortex.add_to_memory(response, metadata={"context": "assistant_response"})
    
    # Log the response
    logger.info(f"Response to user query '{user_query}': {response}")
