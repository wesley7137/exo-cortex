...
class Comms():
    ...
    def create_esp_now_connection(self):
        """Initialize ESP-NOW communication"""
        self.esp_now = ESPNowConnection()
        return self.esp_now
...
class WebSocketConnection:
    ...
    def connect(self):
        self.connected = self.ws.connect()
...
class ESPNowConnection:
    ...
    def add_peer(self, mac_address):
        """Add a peer device using its MAC address"""
        try:
    ...
    def send(self, data, peer=None):
        """Send data to a specific peer or all peers"""
        try:
            if peer:
                return self.esp_now.send(peer, data)
            else:
                # Send to all peers
                success = True
                for peer in self.peers:
                    if not self.esp_now.send(peer, data):
                        success = False
                return success
        except Exception as e:
    ...
    def recv(self, timeout_ms=None):
        ...
        try:
            return self.esp_now.recv(timeout_ms)
        except Exception as e:
